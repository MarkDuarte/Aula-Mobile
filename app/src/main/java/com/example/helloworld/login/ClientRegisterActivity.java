...
public class ClientRegisterActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        authListener = firebaseAuth -> {
            ...
            if(user != null){
                Intent intent = new Intent(ClientRegisterActivity.this, FeedActivity.class);
    ...
    private void onRegister(View view) {
        ...
        userListener.addOnCompleteListener(ClientRegisterActivity.this,task -> {
            if(!task.isSuccessful()){
                Toast.makeText(ClientRegisterActivity.this,R.string.sing_up_error,Toast.LENGTH_SHORT).show();
            }else{
                final FirebaseUser user = mAuth.getCurrentUser();
                DatabaseReference db = FirebaseDatabase.getInstance().getReference().child("Users").child("Client").child(user.getUid());
                db.setValue(true).addOnCompleteListener(saved ->{
                    if(!saved.isSuccessful()){
                        user.delete();
                        Toast.makeText(ClientRegisterActivity.this,R.string.sing_up_error,Toast.LENGTH_LONG).show();

                    }
                    Toast.makeText(ClientRegisterActivity.this,R.string.sing_up_success,Toast.LENGTH_SHORT).show();
                    Intent in = new Intent(ClientRegisterActivity.this, FeedActivity.class);
                    startActivity(in);
                    finish();
                });
